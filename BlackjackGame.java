...
public class BlackjackGame {
    ...
    public void initializeGame() {
        String names;
        System.out.println("Welcome to Hitchhiker's Blackjack!");
        System.out.println();
        System.out.println("  HITCHHIKER'S BLACKJACK RULES: ");
        System.out.println("    -Each player is dealt 2 cards. The dealer is dealt 2 cards with 1 face-up and 1 face-down.");
        ...
        System.out.println("    -Dealer “Hits” until they equal or exceed 34.");
        System.out.println("    -The goal is to have a higher card total than the dealer without going over 42.");
        System.out.println("    -If the player total equals the dealer total, it is a “Push” and the hand ends.");
        System.out.println("    -Players win their bet if they beat the dealer. Players win 2.5x their bet if they get “Hitchhiker's Blackjack” which is 42.");
        System.out.println("    -The deck in Hitchhiker's Blackjack is larger than normal; there are 8 suits, numbered cards go up to 15, and there are 3 more face cards");
        System.out.println(); //changed rules to reflect changes

        // Gets the amount of players and creates them
        do {
            System.out.print("How many people are playing (1-10)? ");
            users = ki.nextInt();


        } while (users > 10 || users < 0);
    ...
    }// changed all instances of 21 to 42
    ...
    public void hitOrStand() {
        ...
        for (int i = 0; i < users; i++) {
            if (players[i].getBet() > 0) {
                ...
                } while (c != 'S' && players[i].getTotal() <= 42);
            }
        }
    }
    ...
    public void dealerPlays() {
        ...
        for (int i = 0; i < users && !isSomePlayerStillInTheGame; i++) {
            if (players[i].getBet() > 0 && players[i].getTotal() <= 42) {
        ...
        for (int i = 0; i < users; i++) {
            if (players[i].getBet() > 0) {
                if (players[i].getTotal() > 42) {
                    ...
                    players[i].push();
                } else if (players[i].getTotal() < dealer.calculateTotal() && dealer.calculateTotal() <= 42) {
                    System.out.println(players[i].getName() + " has lost");
                    players[i].loss();
                } else if (players[i].getTotal() == 42) {
    ...
    }//changed all instances of 21 to 42, and 16 to 36
